...
if ( ! empty( $letter_spacing) ) {
	$css_letter_spacing = 'letter-spacing:'.$letter_spacing.'px;';
}

if('yes' == $atts['use_theme_fonts'] && ! empty( $custom_font_weight ) ){
	$css_custom_font_weight = 'font-weight:'.$custom_font_weight.';';
}
if( $position_relative == 'yes' ){
	$css_position_relative = 'position:relative;';
}
if ( ! empty( $styles ) ) {
	$style = 'style="' .$css_letter_spacing. $css_custom_font_weight . $css_position_relative . esc_attr( implode( ';', $styles ) ) . '"';
} else {
	$style = $css_letter_spacing . $css_custom_font_weight . $css_position_relative;
}

if( $splitting_animation == 'yes' ){
	$css_splitting_animation = 'data-splitting';
	$css_wow_class = 'wow';
} else {
	$css_splitting_animation = '';
	$css_wow_class = '';
}
...
if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) ) {
	$output .= '<div class="' . esc_attr( $css_class ) . ' '.$css_wow_class.'" ' . implode( ' ', $wrapper_attributes ).' '.$css_splitting_animation.'>';
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' >';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
	$output .= '</div>';
} else {
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' class="' . esc_attr( $css_class ) . ' '.$css_wow_class.'" ' . implode( ' ', $wrapper_attributes ) .' '.$css_splitting_animation.'>';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
}
